...
import { SignupCommand } from '../src/server-signup.js'

const afters: (() => any)[] = []

// test.before('setup', async (t) => {
// 	// create a key that zecret can load as part of its key authority
// 	await $`rm -rf ~/.zecret/.ssh`.nothrow()
// 	await $`mkdir -p ~/.zecret/.ssh`.nothrow()
// 	// in practice zecret should only have the public key
// 	// but for tests we'll read in the private in order to decode responses
// 	await $`ssh-keygen -t rsa -q -f "~/.zecret/.ssh/JAForbes" -N ""`
// 	await $`ssh-keygen -t rsa -q -f "~/.zecret/.ssh/JBravoe" -N ""`
// 	await $`ssh-keygen -t rsa -q -f "~/.zecret/.ssh/zecret" -N ""`
...
test('server', async (t) => {
	...
	state.config = { ...state.config, BASE_URL: 'https://zecret.fly.dev' }

	// todo-james do sign up first, can't rely on 01's setup/teardown
	...
	assert(loginResponse.tag === 'LoginResponseErr')

	assert(loginResponse.value.message.includes('No user exists'))

	// let signupResponse = await SignupCommand({
	// 	tag: 'SignupCommand',
	// 	value: {
